...
const ArtForm = () => {
  ...
  // destructure out the name and cose being returned from our selector function
  ...
  const handlePriceChange = (event) => {
    // event.target.value is going to come in as a string not a number
  ...
  const handleSubmit = (event) => {
    // we don't want page reload
    event.preventDefault()
    // we need to dispach name and price in our paylaod
    ...
    <form>
      <div>
        ...
        <div className="flex flex-col justify-end ml-3">
          <button className="px-3 py-1 rounded bg-slate-500 text-white">Add To Collection</button>
        </div>
      </div>
    </form>
  </div>
